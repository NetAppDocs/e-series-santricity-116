...
. In Unified Manager, you load the SANtricity OS software file and the NVSRAM file into the repository (an area of the Web Services Proxy server where files are stored). You can add files either from menu:Upgrade Center[Upgrade SANtricity OS Software] or from menu:Upgrade Center[Manage Software Repository].
